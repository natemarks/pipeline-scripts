...
-r, --release_version  cache_clone version, default: 0.31.3
-d  --directory   directory to download and extract cache_clone
EOF
  exit
}
...
function download() {
  ...
  readonly OS
  if [ "${OS}" == "Darwin" ]; then
    TARBALL="cache_clone_darwin_amd64"
  else
    TARBALL="cache_clone_linux_amd64"
  fi
  ...
  mkdir -p "${2}/${1}"
  curl -L "https://github.com/natemarks/cache_clone/releases/download/v${1}/${TARBALL}" \
  -o "${2}/${1}/cache_clone" \
  --silent
  chmod 755 "${2}/${1}/cache_clone"
}
